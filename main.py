...
def write_msg(user_id, message, var_1):
    ...
    if var_1 == "Нет процесса":
        # Случаи когда ответ пользователя не требуесться
        if message == "Не понимаю о чем вы...":
            ...
            message = "Укажите свой адрес и через <</>> Название Банка"
            vk.method('messages.send', {'user_id': user_id, 'message': message, "random_id": random_id})
            return "Банкомат"
        else:
            vk.method('messages.send', {'user_id': user_id, 'message': message, "random_id": random_id})
        ...
        # Если нужно найти ближайший банкомат
        try:
            if "/" in message:
            ...
            spn = rast / 151
            sred_coord = [str((float(mes[0]) + float(coord[0])) / 2), str((float(mes[1]) + float(coord[1])) / 2)]
            image = f"https://static-maps.yandex.ru/1.x/?ll={sred_coord[0]},{sred_coord[1]}&spn={spn},{spn}&l=map&pt={str(coord[0])},{str(coord[1])},flag~{mes[0]},{mes[1]},comma"
            global upload
...
# Загрузчик фото в оперативную память
...
# Функция отправляет фотографии
def send_photo(vk, peer_id, owner_id, photo_id, access_key, adres, bank):
    attachment = f'photo{owner_id}_{photo_id}_{access_key}'
    vk.method('messages.send',
              {'user_id': peer_id, 'message': f"Ближайший банкомат {bank} находиться по адресу: {adres}",
               "random_id": random.randint(0, 100000000),
